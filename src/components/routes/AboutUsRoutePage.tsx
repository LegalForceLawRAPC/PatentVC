...
export function AboutUsRoutePage() {
  return (
    <PageShell activePath="/about-us">
      <RouteBanner title={aboutPage.title} />
      <section className="grid gap-10 pb-[56px] pt-0 lg:grid-cols-[707px_315px] lg:items-start lg:justify-between lg:gap-[139px]">
        <div className="space-y-[26px] font-gilroy-semibold text-[20px] leading-[26.6px] text-[#e8e8e8]">
          {aboutPage.paragraphs.map((paragraph, index) => (
            <p key={paragraph}>
              {index === 0 ? (
                <>
                  PatentVC is a spinout of{" "}
                  <span className="text-[#ffcba7] underline decoration-solid underline-offset-2">
                    Trademarkia.com
                  </span>
                  , dedicated to co-investing with leading Silicon Valley
                  incubators such as YCombinator and Series A investors.
                </>
              ) : (
                paragraph
              )}
        ...
        <Image
          src="/patentvc/about-mark.png"
          alt="PatentVC mark card"
          width={315}
          height={288}
          className="mx-auto h-auto w-[315px]"
        />
      </section>
        ...
        <div className="rounded-[20px] bg-[#1d1e20] px-[40px] py-[49px] lg:px-[231px] lg:py-[49px]">
          <div className="text-center">
            <h2 className="font-gilroy-bold text-[32px] leading-[38.4px] tracking-[-0.02em] text-white">
              {aboutPage.locationsTitle}{" "}
              <span className="text-[#ffcba7]">
                {aboutPage.locationsHighlight}
              </span>
            </h2>
            <p className="mt-[9px] font-gilroy-medium text-[20px] leading-[26.6px] text-[#e8e8e8]">
              {aboutPage.locationsSubtitle}
            </p>
          </div>
